...
const (
	...
	ChannelFacebookPersonal = "facebook_personal"
)
...
func (m *Manager) Update(id int, inbox imodels.Inbox) (imodels.Inbox, error) {
	...
	case ChannelZaloPersonal, ChannelFacebookPersonal:
		var currentCfg, updateCfg map[string]any
		if err := json.Unmarshal(current.Config, &currentCfg); err != nil {
			return imodels.Inbox{}, fmt.Errorf("unmarshalling current Zalo config: %w", err)
		}
		if err := json.Unmarshal(inbox.Config, &updateCfg); err != nil {
			return imodels.Inbox{}, fmt.Errorf("unmarshalling updated Zalo config: %w", err)
		}
		if token, _ := updateCfg["connector_token"].(string); token == "" || strings.Contains(token, stringutil.PasswordDummy) {
			updateCfg["connector_token"] = currentCfg["connector_token"]
		}
		updatedConfig, err := json.Marshal(updateCfg)
		if err != nil {
			return imodels.Inbox{}, fmt.Errorf("marshalling updated Zalo config: %w", err)
		}
		inbox.Config = updatedConfig
	}
...
func (m *Manager) encryptInboxConfig(config json.RawMessage) (json.RawMessage, error) {
	...
	// Encrypt a Zalo connector token if present.
	if token, ok := cfg["connector_token"].(string); ok && token != "" {
		encrypted, err := crypto.Encrypt(token, m.encryptionKey)
		if err != nil {
			return nil, fmt.Errorf("encrypting connector token: %w", err)
		}
		cfg["connector_token"] = encrypted
	}
...
func (m *Manager) decryptInboxConfig(config json.RawMessage) (json.RawMessage, error) {
	...
	if token, ok := cfg["connector_token"].(string); ok && token != "" {
		decrypted, err := crypto.Decrypt(token, m.encryptionKey)
		if err != nil {
			m.lo.Error("error decrypting connector token, clearing field", "error", err)
			cfg["connector_token"] = ""
		} else {
			cfg["connector_token"] = decrypted
		}
	}
