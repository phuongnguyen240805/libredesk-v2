...
type AutomationRunRecord struct {
	...
	ConversationID *string `db:"conversation_id,omitempty" json:"conversation_id,omitempty"`
	Status         string  `db:"status" json:"status"`
	ErrorSummary   *string `db:"error_summary,omitempty" json:"error_summary,omitempty"`
	StartedAt      string  `db:"started_at" json:"started_at"`
	FinishedAt     *string `db:"finished_at,omitempty" json:"finished_at,omitempty"`
}

// GetAutomationRuns returns recent automation execution logs for debugging.
func (e *Engine) GetAutomationRuns(limit int) ([]AutomationRunRecord, error) {
	if limit <= 0 {
		limit = 20
	}
	...
	query := `
		SELECT id, workspace_id, workflow_id, trigger_event, conversation_id::text, status, error_summary, started_at::text, finished_at::text
		FROM automation_runs
		ORDER BY started_at DESC
		LIMIT $1;
	`
	err := e.db.Select(&runs, query, limit)
	return runs, err
}
