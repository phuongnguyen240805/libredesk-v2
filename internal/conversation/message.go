...
func (m *Manager) sendOutgoingMessage(message models.Message) {
	...
	// Convert to OutboundMessage for transport. Channel metadata lives on the
	// conversation so every agent reply can reach the original external thread.
	outbound := message.ToOutbound()
	if inb.Channel() != inbox.ChannelEmail {
		conversation, convErr := m.GetConversation(message.ConversationID, "", "")
		if convErr != nil {
			handleError(convErr, "error fetching conversation transport metadata")
			return
		}
		outbound.Meta = mergeJSONObjects(conversation.Meta, outbound.Meta)
	}
...
// mergeJSONObjects merges two JSON objects. Values from overlay take precedence.
// Invalid or empty payloads are treated as empty objects because message metadata
// must never prevent a channel reply from being delivered.
...
func (m *Manager) RenderMessageInTemplate(channel string, message *models.Message) error {
	...
	case inbox.ChannelLiveChat, inbox.ChannelZaloPersonal, inbox.ChannelFacebookPersonal:
		// Live chat and Zalo transport plain message content without an email template.
		return nil
	default:
...
// GetConversationUUIDBySourceID returns the conversation UUID for an external
// source message ID. It is used by idempotent connector webhooks.
func (m *Manager) GetConversationUUIDBySourceID(sourceID string) (string, error) {
	conversationID, err := m.messageExistsBySourceID([]string{sourceID})
	if err != nil {
		return "", err
	}
	return m.GetConversationUUID(conversationID)
}
