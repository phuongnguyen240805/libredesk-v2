...
func initHandlers(g *fastglue.Fastglue, hub *ws.Hub) {
	...
	// Zalo connector webhook. Authentication is performed inside the handler
	// using the per-inbox connector token rather than an agent session.
	g.POST("/api/v1/channels/zalo/inbound", handleZaloInbound)
	g.POST("/api/v1/channels/facebook/inbound", handleFacebookInbound)
	...
	// Deplao CSKH Integration API routes.
	g.GET("/api/v1/automations/runs", perm(handleGetAutomationRuns, "automations:manage"))
	g.GET("/api/v1/outbox/status", perm(handleGetOutboxStatus, "automations:manage"))
	g.GET("/api/v1/scheduled-jobs", perm(handleGetScheduledJobs, "automations:manage"))
	g.GET("/api/v1/events/status", perm(handleGetEventPipelineStatus, "automations:manage"))
	g.GET("/api/v1/conversations/{id}/suggestions", auth(handleGetAISuggestions))
	g.GET("/api/v1/contacts/{id}/identities", auth(handleGetContactIdentities))
