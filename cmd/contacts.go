...
// handleGetContactIdentities returns multi-channel identities for a contact in Customer 360 profile.
func handleGetContactIdentities(r *fastglue.Request) error {
	var (
		app          = r.Context.(*App)
		contactIDStr = r.RequestCtx.UserValue("id").(string)
	)
	identityStore := user.NewIdentityStore(app.user.DB())
	identities, err := identityStore.GetContactIdentities(r.RequestCtx, "00000000-0000-0000-0000-000000000000", contactIDStr)
	if err != nil {
		return sendErrorEnvelope(r, err)
	}
	return r.SendEnvelope(identities)
}
