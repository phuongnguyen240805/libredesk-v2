...
func validateInbox(app *App, inbox imodels.Inbox) error {
	...
	// Validate Zalo personal connector config.
	if inbox.Channel == zalo.ChannelZaloPersonal {
		var cfg zalo.Config
		if err := json.Unmarshal(inbox.Config, &cfg); err != nil {
			return envelope.NewError(envelope.InputError, "Invalid Zalo connector config", nil)
		}
		cfg.ConnectorURL = strings.TrimRight(strings.TrimSpace(cfg.ConnectorURL), "/")
		cfg.ConnectorToken = strings.TrimSpace(cfg.ConnectorToken)
		cfg.AccountID = strings.TrimSpace(cfg.AccountID)
		parsedURL, err := url.Parse(cfg.ConnectorURL)
		if err != nil || parsedURL.Host == "" || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") {
			return envelope.NewError(envelope.InputError, "connector_url must be a valid HTTP or HTTPS URL", nil)
		}
		if cfg.AccountID == "" {
			return envelope.NewError(envelope.InputError, "account_id is required", nil)
		}
		if len(cfg.ConnectorToken) < 24 {
			return envelope.NewError(envelope.InputError, "connector_token must contain at least 24 characters", nil)
		}
		if cfg.RequestTimeout != "" {
			timeout, err := time.ParseDuration(cfg.RequestTimeout)
			if err != nil || timeout <= 0 || timeout > 2*time.Minute {
				return envelope.NewError(envelope.InputError, "request_timeout must be between 1ns and 2m", nil)
			}
		}
	}

	if inbox.Channel == facebook.ChannelFacebookPersonal {
		var cfg facebook.Config
		if err := json.Unmarshal(inbox.Config, &cfg); err != nil {
			return envelope.NewError(envelope.InputError, "Invalid Facebook connector config", nil)
		}
		cfg.ConnectorURL = strings.TrimRight(strings.TrimSpace(cfg.ConnectorURL), "/")
		parsedURL, err := url.Parse(cfg.ConnectorURL)
		if err != nil || parsedURL.Host == "" || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") {
		...
		if strings.TrimSpace(cfg.AccountID) == "" || len(strings.TrimSpace(cfg.ConnectorToken)) < 24 {
			return envelope.NewError(envelope.InputError, "Facebook account_id and connector_token are required", nil)
		}
	}
...
func trimInboxFields(inb *imodels.Inbox) error {
	...
	if inb.Channel == inbox.ChannelZaloPersonal && len(inb.Config) > 0 {
		var cfg zalo.Config
		...
		cfg.ConnectorToken = strings.TrimSpace(cfg.ConnectorToken)
		cfg.AccountID = strings.TrimSpace(cfg.AccountID)
		cfg.RequestTimeout = strings.TrimSpace(cfg.RequestTimeout)
	...
	if inb.Channel == inbox.ChannelFacebookPersonal && len(inb.Config) > 0 {
		var cfg facebook.Config
		if err := json.Unmarshal(inb.Config, &cfg); err != nil {
			return err
		}
