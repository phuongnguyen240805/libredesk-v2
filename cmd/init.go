...
// initZaloInbox initializes a Zalo personal inbox backed by the zca-js connector.
func initZaloInbox(inboxRecord imodels.Inbox, msgStore inbox.MessageStore, usrStore inbox.UserStore) (inbox.Inbox, error) {
	var config zalo.Config

	if err := json.Unmarshal(inboxRecord.Config, &config); err != nil {
		return nil, fmt.Errorf("unmarshalling `%s` %s config: %w", inboxRecord.Channel, inboxRecord.Name, err)
	}

	inb, err := zalo.New(msgStore, usrStore, zalo.Opts{
		ID:     inboxRecord.ID,
		Name:   inboxRecord.Name,
		Config: config,
		Lo:     initLogger("zalo_inbox"),
	})
	if err != nil {
		return nil, fmt.Errorf("initializing `%s` inbox: `%s` error: %w", inboxRecord.Channel, inboxRecord.Name, err)
	}
...
func initFacebookInbox(inboxRecord imodels.Inbox, msgStore inbox.MessageStore, usrStore inbox.UserStore) (inbox.Inbox, error) {
	var config facebook.Config
	if err := json.Unmarshal(inboxRecord.Config, &config); err != nil {
		return nil, fmt.Errorf("unmarshalling `%s` %s config: %w", inboxRecord.Channel, inboxRecord.Name, err)
	}
	inb, err := facebook.New(msgStore, usrStore, facebook.Opts{ID: inboxRecord.ID, Name: inboxRecord.Name, Config: config, Lo: initLogger("facebook_inbox")})
	if err != nil {
		return nil, fmt.Errorf("initializing `%s` inbox: `%s` error: %w", inboxRecord.Channel, inboxRecord.Name, err)
	}
	log.Printf("`%s` inbox successfully initialized", inboxRecord.Name)
...
func makeInboxInitializer(mgr *inbox.Manager, signAvatarURL func(*null.String)) func(imodels.Inbox, inbox.MessageStore, inbox.UserStore) (inbox.Inbox, error) {
	return func(inboxR imodels.Inbox, msgStore inbox.MessageStore, usrStore inbox.UserStore) (inbox.Inbox, error) {
		...
		case inbox.ChannelZaloPersonal:
			return initZaloInbox(inboxR, msgStore, usrStore)
		case inbox.ChannelFacebookPersonal:
			return initFacebookInbox(inboxR, msgStore, usrStore)
		default:
