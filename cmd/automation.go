...
// handleGetAutomationRuns returns recent workflow execution logs for debugging
func handleGetAutomationRuns(r *fastglue.Request) error {
	var (
		app = r.Context.(*App)
	)
	runs, err := app.automation.GetAutomationRuns(50)
	if err != nil {
		return sendErrorEnvelope(r, err)
	}
	return r.SendEnvelope(runs)
}
